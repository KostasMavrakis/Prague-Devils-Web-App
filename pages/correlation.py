...
from data_loader import load_chart_data
...
# Callback to update the card and the line chart based on the filters
@callback(
...
def update_components(y_column):
   ...
   if y_column == "Y: Percentage of Italians in the Team":
       text_values = (df[y_column] * 100).round(1).astype(str) + "%"  # format as percentage
   else:
       text_values = df[y_column]
